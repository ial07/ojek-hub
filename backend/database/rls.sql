...
-- Workers can insert own profile during onboarding
CREATE POLICY "worker_profiles_insert_own" ON worker_profiles
    FOR INSERT WITH CHECK (user_id = auth.uid());

-- Employers can view worker profiles in their queue
CREATE POLICY "worker_profiles_select_employer" ON worker_profiles
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM order_queue oq
            JOIN orders o ON oq.order_id = o.id
            WHERE oq.worker_id = worker_profiles.user_id
            AND o.employer_id = auth.uid()
        )
    );
...
CREATE POLICY "orders_select_worker" ON orders
    FOR SELECT USING (
        status = 'open' AND
        worker_type = (
            SELECT worker_type FROM worker_profiles WHERE user_id = auth.uid()
        )
    );
